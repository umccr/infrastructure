...
"""
Sync ICAv1 JWT to DataBricks

Copies over Portal ICAv1 JWT to DataBricks
...
# Globals
DATABRICKS_SCOPE = "ica"
DATABRICKS_SECRET_NAME = "icav1_service_user_access_token"
...
def handler(event, context):
    """
    Collect ICAv1 Access token value
    Run DataBricks CLI to update secret

    Event contains the following parameters

    ICAv1 Access Token Secrets Manager ARN
    ICA_ACCESS_TOKEN_SECRET_ARN =
    """

    # Get path from environment
    ica_access_token_secret_arn = event.get("ICA_ACCESS_TOKEN_SECRETS_MANAGER_ARN", None)
    ...
    # Check access token
    if ica_access_token_secret_arn is None:
        logging.error("Could not get env var ICA_ACCESS_TOKEN_SECRET_ARN")
    ...
    ica_access_token: str = get_secret(ica_access_token_secret_arn)
