...
# NOTE: Installation requires S# access granted through via stackstorm_instance_profile.
...
# NOTE: Change to create separate profile for packer, which is stable and independent of Terraform setup. Will allow restriction of Terraform profile to specific buckets.
docker plugin install --grant-all-permissions rexray/s3fs S3FS_REGION=ap-southeast-2 REXRAY_LOGLEVEL=debug
