...
inotifywait -m -r --exclude "[^c][^s][^v]$" $DIR -e create -e moved_to |
    while read path action file; do
        if test "$file" = "$TARGET_FILE"; then
          ...
          echo "$DEPLOY_ENV"
          source $HOME/.bashrc
          AWS_PROFILE=sample_monitor aws lambda invoke --invocation-type RequestResponse --function-name bootstrap_slack_lambda_dev --region ap-southeast-2 --payload "{ \"topic\": \"Incoming run monitor\", \"title\": \"New runfolder detected\", \"message\": \"$path\"}" /dev/null
        fi
    done