terraform {
  backend "s3" {
    ...
    key            = "longranger/terraform.tfstate"
...
resource "aws_batch_job_definition" "longranger_standard" {
...
data "template_file" "additionalEc2InstancePolicy" {
  ...
  vars {
    resources = "${jsonencode(var.workspace_longranger_buckets[terraform.workspace])}"
  }
}
...
data "template_file" "lambda" {
    template = "${file("${path.module}/policies/longranger-lambda.json")}"
}
...
module "lambda" {
  ...
  source_path   = "${path.module}/lambdas/longranger.py"
  ...
  environment {
    variables {
      ...
      JOBDEF   = "${aws_batch_job_definition.longranger_standard.arn}"
      BUCKET   = "${var.workspace_longranger_data_bucket[terraform.workspace]}"
    }
  }
