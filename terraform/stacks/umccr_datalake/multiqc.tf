locals {
  ...
  s3_multiqc_base = "s3://${data.aws_s3_bucket.datalake.bucket}/${local.datalake_version}/multiqc"
}

resource "aws_glue_catalog_database" "multiqc" {
  name = "multiqc"
}

resource "aws_glue_crawler" "multiqc_crawlers" {
  for_each = local.multiqc_tables

  database_name = aws_glue_catalog_database.multiqc.name
  name          = "${each.value}_multiqc_crawler"
  ...
  s3_target {
    ...
    path = "${local.s3_multiqc_base}/${each.value}"
