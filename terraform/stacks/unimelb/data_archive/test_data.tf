...
resource "aws_iam_user" "icav2_test_data_admin" {
  ...
  path = "/icav2_test_data/"
  tags = local.default_tags
}

resource "aws_iam_group" "icav2_test_data" {
  name = "icav2_test_data"
  path = "/icav2_test_data/"
}

resource "aws_iam_group_membership" "icav2_test_data" {
  name  = "${aws_iam_group.icav2_test_data.name}_membership"
  group = aws_iam_group.icav2_test_data.name
  users = [
    aws_iam_user.icav2_test_data_admin.name,
  ]
}

resource "aws_iam_group_policy_attachment" "icav2_test_data_group_policy_attachment" {
  group      = aws_iam_group.icav2_test_data.name
  policy_arn = aws_iam_policy.icav2_test_data_user_policy.arn
}

data "aws_iam_policy_document" "icav2_test_data_user_policy" {
  # see: https://help.ica.illumina.com/home/h-storage/s-awss3#id-2-create-data-access-permission-aws-iam-policy
  statement {
    actions = sort([
...
resource "aws_iam_policy" "icav2_test_data_user_policy" {
  name   = "icav2_test_data_user_policy"
  path   = "/icav2_test_data/"
  policy = data.aws_iam_policy_document.icav2_test_data_user_policy.json
}
